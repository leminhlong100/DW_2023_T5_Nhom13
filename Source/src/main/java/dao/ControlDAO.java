...
public class ControlDAO {
    ...
    public static Optional<Control> checkRecord(String name) {
        try (Handle handle = DBContext.me().open()) {
            String query = "SELECT id, create_at, status FROM control WHERE DATE(create_at) = ? AND status = 'SFS' AND name = ? LIMIT 1";
            LocalDate currentDate = LocalDate.now();
            return handle.createQuery(query)
                    .bind(0, currentDate)
                    .bind(1, name) // Thêm dòng này để bind tham số name vào câu truy vấn
